...
<head>
    ...
    <title>Match STYLE | Registrasi</title>
...
<body class="font-['Poppins'] min-h-screen flex items-center justify-center relative z-0 p-4"
    ...
    <div class="w-full max-w-md bg-white/70 backdrop-blur-xl border border-white/60 rounded-[2rem] shadow-2xl shadow-pink-900/10 p-8 md:p-10 relative z-10 animate-fade-in-up mt-10 mb-10">
        
        <div class="flex justify-center mb-6">
            <img src="{{ asset('logo.png') }}" alt="Match STYLE" class="h-14 md:h-16 w-auto object-contain drop-shadow-sm">
        </div>

        <div class="text-center mb-8">
            <h2 class="text-2xl font-bold text-[#ca6180] tracking-wide">Buat Akun Baru</h2>
            <p class="text-xs text-[#007979] mt-1 font-medium">Bergabunglah untuk mengatur style-mu.</p>
        </div>

        <form method="POST" action="{{ route('register') }}" class="space-y-5 animate-fade-in-up animate-delay-150">
            @csrf

            <div>
                <label for="name" class="block text-xs font-semibold text-[#007979] mb-1.5 ml-1 tracking-wide">Nama Lengkap</label>
                <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" 
                    class="w-full px-5 py-3.5 bg-white/60 border border-pink-200 rounded-2xl focus:outline-none focus:border-[#ca6180] focus:ring-4 focus:ring-[#ca6180]/10 transition-all text-sm text-gray-700 shadow-sm placeholder-gray-400" placeholder="Masukkan nama kamu">
                @error('name')
            ...
            <div>
                ...
                <input id="password" type="password" name="password" required autocomplete="new-password" 
                    class="w-full px-5 py-3.5 bg-white/60 border border-pink-200 rounded-2xl focus:outline-none focus:border-[#ca6180] focus:ring-4 focus:ring-[#ca6180]/10 transition-all text-sm text-gray-700 shadow-sm placeholder-gray-400" placeholder="Buat password">
                @error('password')
                    <p class="text-[10px] text-red-500 mt-1 ml-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="password_confirmation" class="block text-xs font-semibold text-[#007979] mb-1.5 ml-1 tracking-wide">Konfirmasi Password</label>
                <input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password" 
                    class="w-full px-5 py-3.5 bg-white/60 border border-pink-200 rounded-2xl focus:outline-none focus:border-[#ca6180] focus:ring-4 focus:ring-[#ca6180]/10 transition-all text-sm text-gray-700 shadow-sm placeholder-gray-400" placeholder="Ulangi password">
        ...
        <p class="text-center text-xs text-[#007979] mt-8 font-medium animate-fade-in-up animate-delay-300">
            Sudah punya akun? 
            <a href="{{ route('login') }}" class="text-[#ca6180] font-bold hover:underline underline-offset-4 decoration-2 transition-all">Masuk di sini</a>
        </p>
