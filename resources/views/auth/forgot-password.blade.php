...
<head>
    ...
    <title>Match STYLE | Lupa Password</title>
...
<body class="font-['Poppins'] min-h-screen flex items-center justify-center relative z-0 p-4"
    ...
    <a href="{{ route('login') }}" class="absolute top-6 left-6 z-20 flex items-center gap-2 text-[#007979] hover:text-[#ca6180] transition-colors font-medium text-sm bg-white/50 backdrop-blur-md px-4 py-2 rounded-full shadow-sm border border-white/40">
    ...
    <div class="w-full max-w-md bg-white/70 backdrop-blur-xl border border-white/60 rounded-[2rem] shadow-2xl shadow-pink-900/10 p-8 md:p-10 relative z-10 animate-fade-in-up">
        ...
        <div class="flex justify-center mb-6">
            <img src="{{ asset('logo.png') }}" alt="Logo Match STYLE" class="h-16 md:h-20 w-auto object-contain drop-shadow-sm">
        </div>

        <div class="text-center mb-8">
            <h2 class="text-2xl font-bold text-[#ca6180] tracking-wide mb-2">Lupa Password?</h2>
            <p class="text-xs text-[#007979] font-medium leading-relaxed">
                Jangan khawatir. Masukkan email kamu dan kami akan mengirimkan tautan untuk reset password.
            </p>
        </div>

        @if (session('status'))
            <div class="mb-6 font-medium text-xs text-green-600 bg-green-50 p-3 rounded-xl border border-green-200 text-center">
                {{ session('status') }}
            </div>
        @endif

        <form method="POST" action="{{ route('password.email') }}" class="space-y-6 animate-fade-in-up animate-delay-150">
            @csrf

            <div>
                <label for="email" class="block text-xs font-semibold text-[#007979] mb-1.5 ml-1 tracking-wide">Email</label>
                <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus
                    class="w-full px-5 py-3.5 bg-white/60 border border-pink-200 rounded-2xl focus:outline-none focus:border-[#ca6180] focus:ring-4 focus:ring-[#ca6180]/10 transition-all text-sm text-gray-700 shadow-sm placeholder-gray-400" placeholder="Masukkan email terdaftar">
                @error('email')
                    <p class="text-[10px] text-red-500 mt-1 ml-1 font-medium">{{ $message }}</p>
                @enderror
            </div>

            <div class="pt-2 animate-fade-in-up animate-delay-300">
